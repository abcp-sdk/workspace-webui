...
export function cardFor(
  ...
  switch (tool) {
    ...
    case 'repo-delete-push-mirror': {
      ...
      return {
        subtitle: at || 'push mirror',
        fields: [{ icon: 'branch', label: 'remote', value: name, mono: true }],
      }
    }
    case 'repo-remove': {
      return {
    ...
    case 'sandbox-job-wait': {
      ...
      const fields: CardField[] = []
      if (name)
        fields.push({ icon: 'box', label: 'sandbox', value: name, mono: true })
      if (jobId)
        fields.push({
          icon: 'terminal',
          label: 'job',
    ...
    case 'sandbox-job-stdin': {
      ...
      return {
        subtitle: jobId,
        fields: [{ icon: 'terminal', label: 'job', value: jobId, mono: true }],
      }
    }
    ...
    case 'service-list': {
      ...
      if (!services.length)
        return {
          subtitle: 'services',
          fields: [{ icon: 'server', label: 'count', value: '0' }],
        }
      return {
        subtitle: 'services',
        fields: [
    ...
    case 'sandbox-list': {
      ...
      if (!sandboxes.length)
        return {
          subtitle: 'sandboxes',
          fields: [{ icon: 'box', label: 'count', value: '0' }],
        }
      return {
        subtitle: 'sandboxes',
        fields: [
    ...
    case 'video-generate': {
      ...
      if (prompt)
        fields.push({ icon: 'sparkles', label: 'prompt', value: prompt })
      if (s(data, 'model'))
        fields.push({
          icon: 'server',
          label: 'model',
      ...
      return {
        ...
        fields: [
          ...(name
            ...
            : []),
          ...(desc
            ? [{ icon: 'info', label: 'label', value: desc } as CardField]
            : []),
        ],
    ...
    case 'brave-search': {
      ...
      if (!results.length)
        return {
          ...
          fields: [{ icon: 'search', label: 'query', value: query }],
        }
      return {
        subtitle: query || 'search',
        fields: [
          { icon: 'search', label: 'query', value: query },
