...
export type AvatarLevel = 'org' | 'repo' | 'branch'

/** The fixed, radially-symmetric hexagonal wreath shared by every repo: the
 *  honeycomb lattice is symmetric about both axes, so an annulus (radial band)
 *  yields a symmetric ring of hexes. All hexes stay inside the disc. */
export function honeycombWreath(): HexCell[] {
  const r = HEX_SIZE
  ...
  for (let row = -8; row <= 8; row++) {
    ...
    for (let col = -8; col <= 8; col++) {
      ...
      if (d < 0.26 || d > 0.4) continue
      if (d > 0.5 - r) continue
      out.push({ x, y, r, on: true })
    }
  }
  return out
}
...
// ---- three-level hierarchy (identical to easylab's Flutter ChatAvatar) ----

/** Background comes from the org (falls back to repo, then branch). */
function levelBgSeed(org: string, repo: string, branch: string): string {
  return org || repo || branch
}

/** Foreground keeps the repo hue (falls back to org, then branch) so a single
 *  org's repos keep distinct hues on the same background. */
function levelFgSeed(org: string, repo: string, branch: string): string {
...
function fgRgbFor(
  ...
  const darkRungs = [0.34, 0.28, 0.22, 0.17, 0.12]
  const lightRungs = [0.66, 0.72, 0.78, 0.84, 0.9]
  let best = hsl(fgHue, 0.62, 0.66)
  let bestRatio = -1
  for (const l of [...darkRungs, ...lightRungs]) {
    const c = hsl(fgHue, 0.62, l)
    const ratio = contrastRatio(c, bgRgb)
    if (ratio > bestRatio) {
      bestRatio = ratio
      best = c
  ...
  if (bestRatio < 3.0)
    return luminance(bgRgb) > 0.35 ? [23, 24, 28] : [255, 255, 255]
  return best
}
...
export function avatarSpecFor(
  ...
  const bgRgb = hsl(hueOf(levelBgSeed(org, repo, branch)), 0.6, 0.48)
  const fgRgb = fgRgbFor(hueOf(levelFgSeed(org, repo, branch)), bgRgb)
  let hexes: HexCell[]
  switch (level) {
    case 'org':
      hexes = []
      break
    case 'repo':
      hexes = honeycombWreath()
      break
    default:
      hexes = honeycombCells(branch || `${org}/${repo}/${branch}`, true)
      break
  }
  return { bg: rgb(bgRgb), fg: rgb(fgRgb), hexes }
}
